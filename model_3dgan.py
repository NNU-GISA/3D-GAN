...
class BatchNorm(object):
    ...
    def __call__(self, x, train=True):
        return tf.contrib.layers.batch_norm(x, data_format='NHWC', decay=self.momentum, updates_collections=None,
...
class GAN3D(object):
    ...
    def __init__(self, sess, data_set_path, checkpoint_dir,
        ...
        self.learning_rate_g = 0.025
        self.beta_g = 0.5

        self.learning_rate_d = 0.05
        self.beta_d = 0.5
        ...
        self.d_bn4 = BatchNorm(name='d_bn4')
        self.d_bn5 = BatchNorm(name='d_bn5')
    ...
    def build_model(self):
        ...
        self.input_data = tf.placeholder(tf.float32, [self.batch_size] + input_data_dims, name='real_3d_data')
    ...
    def generator(self, z):
        ...
        with tf.variable_scope("generator") as scope:
            ...
            self.z_, self.h0_w, self.h0_b = linear(z, 512 * 4 * 4 * 4, 'g_h0_lin', with_w=True)  # increase dim of z
            # this first linear layer has a lot of weights
            self.h0 = tf.reshape(self.z_, [-1, 4, 4, 4, 512])  # rearrange the increased dim of z
            h0 = tf.nn.relu(self.g_bn0(self.h0))  # batch normalization + relu

            self.h1, self.h1_w, self.h1_b = deconv3d(h0, [self.batch_size, 8, 8, 8, 256], name='g_h1', with_w=True)
            h1 = tf.nn.relu(self.g_bn1(self.h1))

            h2, self.h2_w, self.h2_b = deconv3d(h1, [self.batch_size, 16, 16, 16, 128], name='g_h2', with_w=True)
            h2 = tf.nn.relu(self.g_bn2(h2))

            h3, self.h3_w, self.h3_b = deconv3d(h2, [self.batch_size, 32, 32, 32, 64], name='g_h3', with_w=True)
            h3 = tf.nn.relu(self.g_bn3(h3))

            h4, self.h4_w, self.h4_b = deconv3d(h3, [self.batch_size, 64, 64, 64, 1], name='g_h4', with_w=True)
    ...
    def train(self):
        ...
        d_optim = tf.train.AdamOptimizer(self.learning_rate_d, beta1=self.beta_d) \
            .minimize(self.d_loss, var_list=self.d_vars)
        g_optim = tf.train.AdamOptimizer(self.learning_rate_g, beta1=self.beta_g) \
            .minimize(self.g_loss, var_list=self.g_vars)

        tf.global_variables_initializer().run()
        # tf.initialize_all_variables().run()

        self.g_sum = tf.merge_summary([self.z_sum, self.d__sum, self.d_loss_fake_sum, self.g_loss_sum])
        self.d_sum = tf.merge_summary([self.z_sum, self.d_sum, self.d_loss_real_sum, self.d_loss_sum])

        self.writer = tf.train.SummaryWriter("./logs", self.sess.graph)

        # load 3d data
        list_3d_data_file = load_data_path(self.data_set_path, load_mode=1)  # load just one category
        ...
        for epoch in range(0, self.epoch):
            for idx in range(0, batch_idxs):
                ...
                errD_real = self.d_loss_real.eval({self.input_data: batch_images})
